...
function love.load()
    ...
    table.insert(game.gameObjects, goblin)
end
...
function spawn()
    if enemys_active < conf._GAME_ENEMY_SLOT_SIZE then
        local x = math.random(0, conf._WINDOW_WIDTH - 16)
        local y = math.random(- 50, -16)
        b = Enemy:new{img=enemyImg, x=x, y=y, speed_x=0,  speed_y=math.random(conf._GAME_ENEMY_SPEED-50, conf._GAME_ENEMY_SPEED+50), width=16, heigth=16, duration=0.5}
